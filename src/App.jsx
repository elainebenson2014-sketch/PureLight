...
function InstructorPortal({ profile, onLogout }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const [b, t, s, p, m] = await Promise.all([
  ...
  return (
    <Shell user={profile} onLogout={onLogout} nav={nav} active={active} setActive={setActive} badge={{ grading: pending }}>
      {loading ? <Spinner /> : (
        <>
...
function StudentPortal({ profile, onLogout }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const [b, t, s, m] = await Promise.all([db.listBooks(), db.listTests(), db.listSubmissions(), db.listMessages()]);
  ...
  return (
    <Shell user={profile} onLogout={onLogout} nav={nav} active={active} setActive={setActive} badge={{ tests: available.length }}>
      {loading ? <Spinner /> : (
        <>
